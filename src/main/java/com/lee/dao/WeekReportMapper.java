...
public interface WeekReportMapper {
    ...
    @Select("select id, name,dep,thisWeek,risk,nextWeek,gain,isDelete,createGmt")
    List<WeekReport> listAll();

    @Select("select id, name,dep,thisWeek,risk,nextWeek,gain,isDelete,createGmt from weekreport where name=#{name} limit 1")
    WeekReport findByName(@Param("name") String name);
